...
char * list_to_html(list_t list)
{
	...
	for (int i = 0;i < list_size(list); i++)
	{
		...
		strcat(text, "<tr> <th>");
		itoa(((struct admin_s *)list_get(list, i))->id, strBuf, 10);
		strcat(text, strBuf);
		strcat(text, "</th> <th> <a href=\"http://127.0.0.1:5000/admins/");
		strcat(text, strBuf);
		strcat(text, "\">");
	...
	strcat(text, "</table> <a href=\"http://127.0.0.1:5000/api/admins/new\">New admin</a> </body> </html>");
	return text;
}
